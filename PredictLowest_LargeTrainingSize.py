...
def main(sig, reg):
    ...
    for n in tqdm(range(navg), desc='avg loop for SF LC',leave=True):
        ...
        b_m, y_test_lower = KRR(X_train = X_train[:2**14], X_test = X_test, 
                                y_train = y_lower_train[:2**14], y_test = ref_y_lower,
    ...
    np.save(f'outs/DPe_predictedbase_largetraining_delML_STO3G_mae.npy',full_maes)
    np.save(f'outs/DPe_predictedbase_largetraining_delML_baseline_mae.npy',base_mae)
